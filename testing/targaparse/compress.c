...
{
    printf("RGB: [%02x, %02x, %02x]\n", cc->red_val, cc->green_val, cc->blue_val);
    int val = (cc->red_val << 16) | (cc->green_val << 8) | cc->blue_val;
    printf("HEX: %06x\n", val);

    return val;
//    return (cc->red_val << 4) | (cc->green_val << 2) | cc->blue_val;   
}
...
int main()
{
    const char* filename = "sweet-tooth.tga";
